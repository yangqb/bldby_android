...
public class MainAdapter extends RecyclerView.Adapter<MainAdapter.BaseViewHolder> {
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        ...
        if (cityInfoModel.getType() == CityInfoModel.TYPE_NORMAL) {
            ...
            if (isShowCode) {
                realHolder.tvCode.setVisibility(View.VISIBLE);
                realHolder.tvCode.setText("+" + cityInfoModel.getExtra().toString());
            } else {
                realHolder.tvCode.setVisibility(View.GONE);
                realHolder.tvCode.setText("");
            }

            realHolder.rlItem.setOnClickListener(new View.OnClickListener() {
                @Override
